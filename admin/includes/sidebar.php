...
// Get current page identifier from global (set by startLayout)
...
if ($conn !== null) {
    $stmt = $conn->prepare("SELECT id, title, icon, icon_svg_path, url, parent_id, section_heading_id, menu_order, page_identifier, is_section_heading FROM admin_menus WHERE menu_type = 'admin' AND is_active = 1 ORDER BY menu_order ASC, title ASC");
    if ($stmt) {
        $stmt->execute();
...
// If no menu items in database, show error message
...
function isMenuActive($url, $currentPath, $pageIdentifier = null, $currentPageIdentifier = null) {
    ...
    $normalizePath = function($path) {
        ...
        $path = trim($path, '/');
        // Remove /admin/ prefix if present for comparison (handles both /admin/ and /project/admin/)
        // First try to match pattern with project folder: /project/admin/...
        $path = preg_replace('#^[^/]+/admin/#', '', $path);
        // Then try to match pattern without project folder: /admin/...
        $path = preg_replace('#^admin/#', '', $path);
    ...
    if (!$pathMatches) {
        ...
        // Only use filename matching if both are settings pages and filenames match
...
// $iconNameOrMenuItem can be a string (icon name) or an array (menu item with 'icon' and 'icon_svg_path' keys)
function getIconSVG($iconNameOrMenuItem, $showError = true) {
    ...
    if ($showError) {
        ...
        return getDefaultIconSVG("Icon '{$displayName}' does not exist. Please add it in the Icons page.");
    }
            ...
            <?php if ($menuItemsEmpty): ?>
                <li class="admin-sidebar__item">
                    <div style="padding: 1rem; color: var(--text-error, #ef4444); background: var(--bg-error, #fee2e2); border-radius: 0.5rem; margin: 0.5rem;">
                        ...
                        Please create menu items in the <a href="/admin/setup/menus.php" style="color: var(--text-error, #ef4444); text-decoration: underline;">Menus page</a>.
                    </div>
                </li>
            <?php else: ?>
            ...
            <li class="admin-sidebar__item admin-sidebar__item--section <?php echo $isSectionItemActive ? 'admin-sidebar__item--active' : ''; ?>">
                <a href="<?php echo htmlspecialchars($sectionItemFinalUrl); ?>" class="admin-sidebar__link">
                    <span class="admin-sidebar__icon"<?php if ($showCurrPageTooltip === 'YES' && !empty($sectionItem['page_identifier'])): ?> title="currPage: <?php echo htmlspecialchars($sectionItem['page_identifier']); ?>"<?php endif; ?>>
                        <?php echo getIconSVG($sectionItem); ?>
                    </span>
