/**
 * Reusable Icon Picker JavaScript
 * Provides consistent icon picker functionality across all pages
 */
 ...
 *   - allIcons: Array of all available icons (if not provided, will try to find from global)
 *   - iconSize: Size of icon in pixels (default: 24)
 *   - showText: Whether to show icon name text (default: false)
 *   - onSelect: Callback function to call after selection
 */
function selectIcon(optionElement, iconName, options = {}) {
 ...
 */
function initIconPickerCloseOnOutsideClick() {
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initIconPickerCloseOnOutsideClick);
} else {
    initIconPickerCloseOnOutsideClick();
}
