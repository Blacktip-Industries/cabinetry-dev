...
function detectBaseUrl() {
    ...
    // Remove /admin/components/theme from path
    $base = preg_replace('/\/admin\/components\/theme.*$/', '', $dir);
    if (empty($base) || $base === '/') {
        $base = '';
    }
...
function generateConfigFile($detected) {
    ...
    $config = str_replace("define('THEME_VERSION', '1.0.0');", "define('THEME_VERSION', '{$version}');", $configTemplate);
    $config = str_replace("define('THEME_INSTALLED_AT', '');", "define('THEME_INSTALLED_AT', '" . date('Y-m-d H:i:s') . "');", $config);
    $config = str_replace("define('THEME_DB_HOST', '');", "define('THEME_DB_HOST', '" . addslashes($detected['db']['host']) . "');", $config);
    $config = str_replace("define('THEME_DB_USER', '');", "define('THEME_DB_USER', '" . addslashes($detected['db']['user']) . "');", $config);
    $config = str_replace("define('THEME_DB_PASS', '');", "define('THEME_DB_PASS', '" . addslashes($detected['db']['pass']) . "');", $config);
    $config = str_replace("define('THEME_DB_NAME', '');", "define('THEME_DB_NAME', '" . addslashes($detected['db']['name']) . "');", $config);
    $config = str_replace("define('THEME_ROOT_PATH', '');", "define('THEME_ROOT_PATH', '" . addslashes($detected['paths']['project_root']) . "');", $config);
    $config = str_replace("define('THEME_ADMIN_PATH', '');", "define('THEME_ADMIN_PATH', '" . addslashes($detected['paths']['admin_path']) . "');", $config);
    $config = str_replace("define('THEME_BASE_URL', '');", "define('THEME_BASE_URL', '" . addslashes($detected['base_url']) . "');", $config);
    $config = str_replace("define('THEME_ADMIN_URL', '');", "define('THEME_ADMIN_URL', '" . addslashes($detected['base_url'] . '/admin') . "');", $config);
    $config = str_replace("define('THEME_BASE_SYSTEM_INFO', '{}');", "define('THEME_BASE_SYSTEM_INFO', '" . addslashes(json_encode($detected['base_system_info'])) . "');", $config);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' || ($isCLI && $isAuto)) {
    ...
    if (!$dbTest['success']) {
        ...
        try {
            ...
            $paramResult = theme_insert_default_parameters($conn);
            ...
            // Step 6: Create default light theme
            $lightThemeConfig = json_encode([
                'name' => 'Light',
                'type' => 'light',
                'colors' => [
                    'primary' => '#FF6C2F',
            ...
            $stmt = $conn->prepare("INSERT INTO theme_themes (theme_name, theme_type, is_active, config_json) VALUES ('Light', 'light', 1, ?) ON DUPLICATE KEY UPDATE config_json = VALUES(config_json)");
            $stmt->bind_param("s", $lightThemeConfig);
            $stmt->execute();
            $stmt->close();
            $installResults['steps_completed'][] = 'Default light theme created';
            ...
            // Step 8: Create menu links (if menu_system is installed)
            require_once __DIR__ . '/install/default-menu-links.php';
            $menuResult = theme_create_menu_links($conn, 'theme', $detected['base_url'] . '/admin');
    ...
    if (!file_exists($cssVarsPath)) {
        ...
        if (file_put_contents($cssVarsPath, $cssVarsContent) === false) {
            ...
            $installResults['steps_completed'][] = 'CSS variables file created';
        }
    }
...
if ($isCLI) {
    ...
    if ($isSilent || $isAuto) {
        ...
        echo "Theme Component Installer\n";
        echo "========================\n\n";
    ...
    <head>
        ...
        <title>Theme Component - Installer</title>
        <style>
    ...
    <body>
        <h1>Theme Component - Installer</h1>
        ...
        <?php elseif ($installResults['success']): ?>
            ...
            <div class="info">
                ...
                <ol>
                    <li>Access the design system preview at: <a href="<?php echo htmlspecialchars($detected['base_url'] ?? ''); ?>/admin/components/theme/admin/preview.php">Design System Preview</a></li>
                    <li>Review theme settings and customize as needed</li>
                    <li>Use theme CSS variables in your components</li>
                </ol>
            </div>
        ...
        <?php else: ?>
            <div class="info">
                ...
                <form method="POST">
                    <button type="submit">Install Theme Component</button>
                </form>
            </div>
        <?php endif; ?>
    </body>
