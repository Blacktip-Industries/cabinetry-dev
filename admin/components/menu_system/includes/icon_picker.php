<?php
/**
 * Menu System Component - Icon Picker
 * Reusable icon picker component with menu_system_ prefix
 */
 ...
 */
function menu_system_prepare_icon_svg($svgContent) {
 ...
 */
function menu_system_render_icon_picker($options) {
    ...
    if (empty($options['allIcons']) || !is_array($options['allIcons'])) {
        // Try to get icons from database
        $sortOrder = $options['iconSortOrder'] ?? 'name';
        $options['allIcons'] = menu_system_get_all_icons($sortOrder);
    }
    ...
    $onSelectCallback = $options['onSelectCallback'] ?? 'menu_system_selectIcon';
    ...
    if ($currentIcon && !empty($currentIcon['svg_path'])) {
        $svgData = menu_system_prepare_icon_svg($currentIcon['svg_path']);
    ...
    // Button
    $html .= '<button type="button" class="icon-picker-button input" onclick="menu_system_toggleIconPicker(this)" style="width: 100%; max-width: 100%; display: flex; align-items: center; justify-content: space-between; padding: 8px 12px; min-height: auto; box-sizing: border-box;">';
