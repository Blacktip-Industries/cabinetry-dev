<?php
/**
 * Order Management Component - Installer
...
function detectBaseUrl() {
    ...
    $base = preg_replace('/\/admin\/components\/order_management.*$/', '', $dir);
    if (empty($base) || $base === '/') {
        $base = '';
    }
...
function generateConfigFile($detected) {
    ...
    $config = str_replace("define('ORDER_MANAGEMENT_DB_HOST', '');", "define('ORDER_MANAGEMENT_DB_HOST', '" . addslashes($detected['db']['host']) . "');", $configTemplate);
    $config = str_replace("define('ORDER_MANAGEMENT_DB_USER', '');", "define('ORDER_MANAGEMENT_DB_USER', '" . addslashes($detected['db']['user']) . "');", $config);
    $config = str_replace("define('ORDER_MANAGEMENT_DB_PASS', '');", "define('ORDER_MANAGEMENT_DB_PASS', '" . addslashes($detected['db']['pass']) . "');", $config);
    $config = str_replace("define('ORDER_MANAGEMENT_DB_NAME', '');", "define('ORDER_MANAGEMENT_DB_NAME', '" . addslashes($detected['db']['name']) . "');", $config);
    $config = str_replace("define('ORDER_MANAGEMENT_BASE_URL', '');", "define('ORDER_MANAGEMENT_BASE_URL', '" . addslashes($detected['base_url']) . "');", $config);
    $config = str_replace("define('ORDER_MANAGEMENT_ENCRYPTION_KEY', '');", "define('ORDER_MANAGEMENT_ENCRYPTION_KEY', '" . addslashes(bin2hex(random_bytes(32))) . "');", $config);
...
if (($isCLI && ($isAuto || $isSilent)) || (!$isCLI && isset($_POST['install']))) {
    ...
    $checks = order_management_run_checks();
    ...
    if (!$testResult['success']) {
        ...
        try {
            ...
            $migrationResult = order_management_migrate_1_0_0($conn);
            if ($migrationResult['success']) {
                $installResults['steps_completed'][] = 'Database migration completed (version ' . $migrationResult['version'] . ')';
            } else {
            ...
            if (empty($installResults['errors'])) {
                $paramResult = order_management_insert_default_parameters($conn);
            ...
            if (empty($installResults['errors'])) {
                ...
                $stmt = $conn->prepare("INSERT INTO order_management_config (config_key, config_value) VALUES ('version', ?), ('installed_at', ?), ('base_system_info', ?) ON DUPLICATE KEY UPDATE config_value = VALUES(config_value)");
            ...
            if (empty($installResults['errors'])) {
                $menuResult = order_management_create_menu_links($conn, 'order_management', $detected['base_url'] . '/admin');
...
if ($isCLI) {
    ...
    if ($isSilent || $isAuto) {
        ...
        echo "Order Management Component Installer\n";
        echo "====================================\n\n";
    ...
    <head>
        ...
        <title>Order Management Component - Installer</title>
        <style>
    ...
    <body>
        <h1>Order Management Component Installer</h1>
        ...
        <?php else: ?>
            <form method="POST">
                ...
                <button type="submit" name="install">Install Order Management Component</button>
            </form>
        <?php endif; ?>
