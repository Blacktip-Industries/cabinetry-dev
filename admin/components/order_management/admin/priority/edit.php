<?php
/**
 * Order Management Component - Edit Priority Level
 */
...
$priorityId = $_GET['id'] ?? 0;
$priority = order_management_get_priority($priorityId);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name)) {
        ...
        $stmt = $conn->prepare("UPDATE {$tableName} SET name = ?, priority_value = ?, color = ?, updated_at = NOW() WHERE id = ?");
        $stmt->bind_param("sisi", $name, $priorityValue, $color, $priorityId);
        if ($stmt->execute()) {
            header('Location: ' . order_management_get_component_admin_url() . '/priority/index.php');
            exit;
        } else {
...
$pageTitle = 'Edit Priority: ' . htmlspecialchars($priority['name']);
...
<div class="order_management__container">
    ...
    <form method="POST" class="order_management__form">
        <div class="order_management__form-group">
            ...
            <input type="text" id="name" name="name" value="<?php echo htmlspecialchars($priority['name']); ?>" required>
        </div>
        
        <div class="order_management__form-group">
            <label for="priority_value">Priority Value *</label>
            <input type="number" id="priority_value" name="priority_value" value="<?php echo $priority['priority_value']; ?>" required>
        </div>
        
        <div class="order_management__form-group">
            <label for="color">Color *</label>
            <input type="color" id="color" name="color" value="<?php echo htmlspecialchars($priority['color'] ?? '#007bff'); ?>" required>
        </div>
