<?php
/**
 * Order Management Component - Edit Template
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name)) {
        ...
        // Update template (would need update function)
        $conn = order_management_get_db_connection();
        $tableName = order_management_get_table_name('order_templates');
        $templateDataJson = json_encode($templateData);
        $stmt = $conn->prepare("UPDATE {$tableName} SET name = ?, description = ?, template_data = ?, updated_at = NOW() WHERE id = ?");
        $stmt->bind_param("sssi", $name, $description, $templateDataJson, $templateId);
        if ($stmt->execute()) {
            header('Location: ' . order_management_get_component_admin_url() . '/templates/view.php?id=' . $templateId);
            exit;
        } else {
...
$pageTitle = 'Edit Template: ' . htmlspecialchars($template['name']);
...
<div class="order_management__container">
    ...
    <form method="POST" class="order_management__form">
        ...
        <div class="order_management__form-group">
            ...
            <textarea id="description" name="description" rows="3"><?php echo htmlspecialchars($template['description'] ?? ''); ?></textarea>
        </div>
        
        <div class="order_management__form-group">
            <label for="template_data">Template Data (JSON)</label>
            <textarea id="template_data" name="template_data" rows="10"><?php echo htmlspecialchars(json_encode($template['template_data'] ?? [], JSON_PRETTY_PRINT)); ?></textarea>
        </div>
        
        <div class="order_management__form-actions">
            <button type="submit" class="btn btn-primary">Save Changes</button>
            <a href="<?php echo order_management_get_component_admin_url(); ?>/templates/view.php?id=<?php echo $templateId; ?>" class="btn btn-secondary">Cancel</a>
        </div>
    </form>
</div>
