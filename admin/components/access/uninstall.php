...
if ($_SERVER['REQUEST_METHOD'] === 'POST' || ($isCLI && isset($argv[1]) && $argv[1] === '--yes')) {
    try {
        ...
        if ($conn) {
            // Backup all access_* tables
            $tables = ['access_config', 'access_parameters', 'access_parameters_configs', 'access_account_types', 
                      'access_account_type_fields', 'access_accounts', 'access_account_data', 'access_users',
                      'access_user_accounts', 'access_roles', 'access_permissions', 'access_role_permissions',
                      'access_user_permissions', 'access_registrations', 'access_sessions', 'access_login_history',
                      'access_workflows', 'access_field_types', 'access_hooks', 'access_email_templates', 'access_audit_log'];
            
            $backupSQL = "-- Access Component Backup\n";
        ...
        if ($conn) {
            ...
            $menuResult = access_remove_menu_links($conn, 'access');
        ...
        if ($conn) {
            $tables = ['access_audit_log', 'access_email_templates', 'access_hooks', 'access_field_types', 
                      'access_workflows', 'access_login_history', 'access_sessions', 'access_registrations',
                      'access_user_permissions', 'access_role_permissions', 'access_permissions', 'access_roles',
                      'access_user_accounts', 'access_users', 'access_account_data', 'access_accounts',
                      'access_account_type_fields', 'access_account_types', 'access_parameters_configs',
                      'access_parameters', 'access_config'];
...
if ($isCLI) {
    echo "Access Component Uninstaller\n";
    echo "============================\n\n";
    ...
    <head>
        ...
        <title>Access Component - Uninstaller</title>
        <style>
    ...
    <body>
        <h1>Access Component - Uninstaller</h1>
        ...
        <?php if ($uninstallResults['success']): ?>
            ...
            <div class="warning">
                <strong>Important:</strong> Component files are still in the filesystem. You may delete the component directory manually if desired.
            </div>
        ...
        <?php else: ?>
            <div class="warning">
                <strong>Warning:</strong> This will remove all access component data, including:
                <ul>
                    <li>All users and accounts</li>
                    <li>All roles and permissions</li>
                    <li>All registration requests</li>
                    <li>All audit logs</li>
                </ul>
                A backup will be created before uninstallation.
            </div>
            
            <form method="POST" onsubmit="return confirm('Are you sure you want to uninstall the Access Component? This action cannot be undone!');">
                <button type="submit">Uninstall Access Component</button>
            </form>
        <?php endif; ?>
    </body>
    </html>
