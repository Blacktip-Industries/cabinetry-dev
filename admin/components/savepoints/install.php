<?php
/**
 * Savepoints Component - Installer
...
function detectBaseUrl() {
    ...
    // Remove /admin/components/savepoints from path
    $base = preg_replace('/\/admin\/components\/savepoints.*$/', '', $dir);
    if (empty($base) || $base === '/') {
        $base = '';
    }
...
function generateConfigFile($detected) {
    ...
    $config = str_replace("define('SAVEPOINTS_VERSION', '1.0.0');", "define('SAVEPOINTS_VERSION', '{$version}');", $configTemplate);
    $config = str_replace("define('SAVEPOINTS_INSTALLED_AT', '');", "define('SAVEPOINTS_INSTALLED_AT', '" . date('Y-m-d H:i:s') . "');", $config);
    $config = str_replace("define('SAVEPOINTS_DB_HOST', '');", "define('SAVEPOINTS_DB_HOST', '" . addslashes($detected['db']['host']) . "');", $config);
    $config = str_replace("define('SAVEPOINTS_DB_USER', '');", "define('SAVEPOINTS_DB_USER', '" . addslashes($detected['db']['user']) . "');", $config);
    $config = str_replace("define('SAVEPOINTS_DB_PASS', '');", "define('SAVEPOINTS_DB_PASS', '" . addslashes($detected['db']['pass']) . "');", $config);
    $config = str_replace("define('SAVEPOINTS_DB_NAME', '');", "define('SAVEPOINTS_DB_NAME', '" . addslashes($detected['db']['name']) . "');", $config);
    $config = str_replace("define('SAVEPOINTS_ROOT_PATH', '');", "define('SAVEPOINTS_ROOT_PATH', '" . addslashes($detected['paths']['project_root']) . "');", $config);
    $config = str_replace("define('SAVEPOINTS_ADMIN_PATH', '');", "define('SAVEPOINTS_ADMIN_PATH', '" . addslashes($detected['paths']['admin_path']) . "');", $config);
    $config = str_replace("define('SAVEPOINTS_BASE_URL', '');", "define('SAVEPOINTS_BASE_URL', '" . addslashes($detected['base_url']) . "');", $config);
    $config = str_replace("define('SAVEPOINTS_ADMIN_URL', '');", "define('SAVEPOINTS_ADMIN_URL', '" . addslashes($detected['base_url'] . '/admin') . "');", $config);
    $config = str_replace("define('SAVEPOINTS_BASE_SYSTEM_INFO', '{}');", "define('SAVEPOINTS_BASE_SYSTEM_INFO', '" . addslashes(json_encode($detected['base_system_info'])) . "');", $config);
...
function generateCSSVariablesFile($detectedCSSVars) {
    ...
    foreach ($detectedCSSVars as $varName => $varValue) {
        $spVarName = '--savepoints-' . str_replace('--', '', $varName);
        $additionalVars .= "    {$spVarName}: var({$varName}, {$varValue});\n";
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' || ($isCLI && $isAuto)) {
    ...
    if (!$dbTest['success']) {
        ...
        try {
            ...
            // Step 5: Insert default parameters and migrate from base system
            require_once __DIR__ . '/install/default-parameters.php';
            $paramResult = savepoints_insert_default_parameters($conn);
            if ($paramResult['success']) {
                $msg = 'Default parameters inserted (' . $paramResult['inserted'] . ' parameters)';
                if ($paramResult['migrated'] > 0) {
                    $msg .= ', migrated ' . $paramResult['migrated'] . ' from base system';
                }
                $installResults['steps_completed'][] = $msg;
            } else {
            ...
            $menuResult = savepoints_create_menu_links($conn, 'savepoints', $detected['base_url'] . '/admin');
...
if ($isCLI) {
    ...
    if ($isSilent || $isAuto) {
        ...
        echo "Savepoints Component Installer\n";
        echo "==============================\n\n";
    ...
    <head>
        ...
        <title>Savepoints Component - Installer</title>
        <style>
    ...
    <body>
        <h1>Savepoints Component - Installer</h1>
        ...
        <?php elseif ($installResults['success']): ?>
            ...
            <div class="info">
                ...
                <ol>
                    <li>Access the savepoints management page at: <a href="<?php echo htmlspecialchars($detected['base_url'] ?? ''); ?>/admin/components/savepoints/admin/index.php">Savepoints Management</a></li>
                    <li>Configure GitHub settings if needed</li>
                    <li>Create your first savepoint</li>
                </ol>
            </div>
        ...
        <?php else: ?>
            <div class="info">
                ...
                <form method="POST">
                    <button type="submit">Install Savepoints Component</button>
                </form>
            </div>
        <?php endif; ?>
    </body>
