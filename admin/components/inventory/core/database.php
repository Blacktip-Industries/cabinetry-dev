<?php
/**
 * Inventory Component - Database Functions
 * All database operations for the inventory component
 */
 ...
 */
function inventory_get_db_connection() {
    static $conn = null;
    ...
    try {
        ...
        if ($conn->connect_error) {
            error_log("Inventory: Database connection failed: " . $conn->connect_error);
            return null;
        }
 ...
 */
function inventory_get_table_name($tableName) {
    $prefix = defined('INVENTORY_TABLE_PREFIX') ? INVENTORY_TABLE_PREFIX : 'inventory_';
 ...
 */
function inventory_is_installed() {
    $conn = inventory_get_db_connection();
    if ($conn === null) {
        return false;
    }
    
    $tableName = inventory_get_table_name('config');
 ...
 */
function inventory_get_version() {
    $conn = inventory_get_db_connection();
    if ($conn === null) {
        return null;
    }
 ...
 */
function inventory_get_parameter($parameterName, $default = null) {
    $conn = inventory_get_db_connection();
    if ($conn === null) {
        return $default;
    }
    
    $tableName = inventory_get_table_name('parameters');
 ...
 * @return bool Success
 */
function inventory_set_parameter($parameterName, $value) {
    $conn = inventory_get_db_connection();
    if ($conn === null) {
        return false;
    }
