...
function detectBaseUrl() {
    ...
    // Remove /admin/components/component_manager from path
    $base = preg_replace('/\/admin\/components\/component_manager.*$/', '', $dir);
    if (empty($base) || $base === '/') {
        $base = '';
    }
...
function runMigration($conn) {
    ...
    return component_manager_migration_1_0_0($conn);
}
...
function insertDefaultParameters($conn) {
    $errors = [];
    $defaultParams = component_manager_get_default_parameters();
    
    foreach ($defaultParams as $param) {
        try {
            $stmt = $conn->prepare("INSERT INTO component_manager_parameters (section, parameter_name, description, value) VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE value = VALUES(value)");
 ...
 * Create menu links
 */
function createMenuLinks($conn, $adminUrl) {
    $result = component_manager_create_menu_links($conn, 'component_manager', $adminUrl);
    return $result;
}
...
function generateConfigFile($detected) {
    ...
    $config = str_replace("define('COMPONENT_MANAGER_VERSION', '1.0.0');", "define('COMPONENT_MANAGER_VERSION', '{$version}');", $configTemplate);
    $config = str_replace("define('COMPONENT_MANAGER_INSTALLED_AT', '');", "define('COMPONENT_MANAGER_INSTALLED_AT', '" . date('Y-m-d H:i:s') . "');", $config);
    $config = str_replace("define('COMPONENT_MANAGER_DB_HOST', '');", "define('COMPONENT_MANAGER_DB_HOST', '" . addslashes($detected['db']['host']) . "');", $config);
    $config = str_replace("define('COMPONENT_MANAGER_DB_USER', '');", "define('COMPONENT_MANAGER_DB_USER', '" . addslashes($detected['db']['user']) . "');", $config);
    $config = str_replace("define('COMPONENT_MANAGER_DB_PASS', '');", "define('COMPONENT_MANAGER_DB_PASS', '" . addslashes($detected['db']['pass']) . "');", $config);
    $config = str_replace("define('COMPONENT_MANAGER_DB_NAME', '');", "define('COMPONENT_MANAGER_DB_NAME', '" . addslashes($detected['db']['name']) . "');", $config);
    $config = str_replace("define('COMPONENT_MANAGER_ROOT_PATH', '');", "define('COMPONENT_MANAGER_ROOT_PATH', '" . addslashes($detected['paths']['project_root']) . "');", $config);
    $config = str_replace("define('COMPONENT_MANAGER_ADMIN_PATH', '');", "define('COMPONENT_MANAGER_ADMIN_PATH', '" . addslashes($detected['paths']['admin_path']) . "');", $config);
    $config = str_replace("define('COMPONENT_MANAGER_BASE_URL', '');", "define('COMPONENT_MANAGER_BASE_URL', '" . addslashes($detected['base_url']) . "');", $config);
    $config = str_replace("define('COMPONENT_MANAGER_ADMIN_URL', '');", "define('COMPONENT_MANAGER_ADMIN_URL', '" . addslashes($detected['base_url'] . '/admin') . "');", $config);
    $config = str_replace("define('COMPONENT_MANAGER_BASE_SYSTEM_INFO', '{}');", "define('COMPONENT_MANAGER_BASE_SYSTEM_INFO', '" . addslashes(json_encode($detected['base_system_info'])) . "');", $config);
    ...
    if (!$dbTest['success']) {
        ...
        try {
            ...
            if ($migrationResult['success']) {
                ...
                $paramsResult = insertDefaultParameters($conn);
                if ($paramsResult['success']) {
                    $installResults['steps_completed'][] = 'Default parameters inserted';
                } else {
                    $installResults['warnings'] = array_merge($installResults['warnings'], $paramsResult['errors']);
                }
                
                // Step 6: Create menu links (if menu_system is installed)
                $menuResult = createMenuLinks($conn, $detected['base_url'] . '/admin');
                if ($menuResult['success']) {
                    $installResults['steps_completed'][] = 'Menu links created';
                } else {
                    // Not an error - menu_system might not be installed
                    $installResults['warnings'][] = 'Menu links not created: ' . ($menuResult['error'] ?? 'menu_system not installed');
                }
            } else {
...
if ($isCLI) {
    ...
    if ($isSilent || $isAuto) {
        ...
        echo "Component Manager Installer\n";
        echo "==========================\n\n";
        ...
        if ($installResults['success']) {
            ...
            if (!empty($installResults['warnings'])) {
                echo "\nWarnings:\n";
                foreach ($installResults['warnings'] as $warning) {
                    echo "  - $warning\n";
                }
            }
        } else {
...
<head>
    ...
    <title>Component Manager - Installation</title>
    <style>
...
<body>
    <h1>Component Manager Installation</h1>
    ...
    <?php else: ?>
        ...
        <p>This installer will set up the Component Manager. All fields are pre-filled with auto-detected values.</p>
        ...
        <form method="POST">
            ...
            <button type="submit">Install Component Manager</button>
        </form>
    <?php endif; ?>
</body>
</html>
