<?php
/**
 * Add Button Border Color Parameter
 * Adds --button-border-color parameter for button border colors
 */
...
startLayout('Add Button Border Color Parameter');

// Get full URL for this script (for clickable links)
$scriptUrl = getAdminUrl('setup/add_button_border_color_parameter.php');
...
if ($conn === null) {
    ...
    $parameterKey = '--button-border-color';
    $parameterValue = '#eaedf1'; // Default border color (same as --border-default)
    $parameterDescription = 'Border color for buttons (secondary buttons, arrow buttons, etc.)';
    ...
    $messages = [];
    if ($added) {
        $messages[] = "Successfully added --button-border-color parameter in '{$targetSection}' section.";
    } else {
        $messages[] = "Parameter --button-border-color already exists in '{$targetSection}' section.";
    }
...
<div class="page-header">
    <div class="page-header__left">
        <h2>Add Button Border Color Parameter</h2>
        <p class="text-muted">Add --button-border-color parameter for button border colors</p>
    </div>
</div>
...
<div class="card">
    <div class="card-body">
        ...
        <p>This script adds the --button-border-color parameter for customizing the border color of buttons (secondary buttons, arrow buttons, etc.).</p>
        <p>The parameter will be added in the "Buttons" section.</p>
        <p>The parameter will be configured to use a color picker with a default color (#eaedf1).</p>
        <?php if ($success): ?>
