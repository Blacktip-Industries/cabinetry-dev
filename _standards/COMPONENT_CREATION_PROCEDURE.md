...
- [ ] **Create menu link creation function** (if component has admin pages)
  - Function to detect menu_system component
  - Function to create menu entries in `menu_system_menus` table
  - Define menu structure (titles, URLs, icons, page identifiers, order)
  - Handle section headings for component grouping
- [ ] **Call menu link creation during installation**
...
### Phase 9.5: Testing Infrastructure

Add comprehensive testing infrastructure to the component:

- [ ] **Copy testing template** from `_standards/TESTING_TEMPLATE/` to component
  - Copy `tests/` directory structure
  - Copy `verify.php` to component root
  - Copy `docs/TESTING.md` template

- [ ] **Customize bootstrap.php**
  - Update component name placeholders (`{COMPONENT_NAME}`, `{component_name}`)
  - Configure component-specific database connection function
  - Add component-specific cleanup functions
  - Load component-specific core files

- [ ] **Customize test data generator**
  - Update `tests/generate_test_data.php` with component-specific table structures
  - Add component-specific data generation logic
  - Configure realistic test data for all component tables
  - Test data generation for relationships and foreign keys

- [ ] **Create unit tests**
  - Test all public functions in `core/` directory
...
- [ ] **Create performance tests** (if applicable)
  - Benchmark critical functions
  - Set performance thresholds
  - Test database query performance

- [ ] **Create security tests** (if applicable)
  - SQL injection tests
  - XSS tests
  - CSRF tests
...
- [ ] **Customize verification script**
  - Update `verify.php` with component-specific tables
  - Add component-specific function checks
  - Add component-specific admin page checks
  - Add component-specific database operation tests

- [ ] **Update TESTING.md documentation**
  - Replace placeholders with component name
  - Add component-specific testing procedures
  - Document component-specific test data requirements
...
- [ ] **Test test data generator**
  - Generate test data: `php tests/generate_test_data.php --generate`
  - Verify data is realistic and complete
  - Test cleanup: `php tests/generate_test_data.php --cleanup`
  - Test regeneration: `php tests/generate_test_data.php --regenerate`
...
Create a function in `install/default-menu-links.php` or similar:
...
In `install.php`, after database setup:

```php
// Step X: Create menu links (if menu_system is installed)
if (empty($installResults['errors'])) {
    require_once __DIR__ . '/install/default-menu-links.php';
    $menuResult = {component_name}_create_menu_links($conn, '{component_name}', $detected['base_url'] . '/admin');
